...
### 5.1. Диалог и безопасное изменение данных

Свободный текст и расшифрованные голосовые сообщения проходят один и тот же путь:

1. сервер передает модели допустимые категории, цели и до 12 последних финансовых операций с временными ссылками `T1`, `T2` и т. д.;
2. модель возвращает одно типизированное намерение: новый расход/доход, отчет, изменение цели, исправление операции или обычный ответ;
3. для исправления модель выбирает ссылку на существующую операцию и область изменения: обычная операция, весь чек или одна позиция;
4. сервер самостоятельно проверяет принадлежность записи семье, допустимость категории, сумму, валюту и состояние финансового месяца;
5. изменение проводится транзакцией: старая ledger-запись отменяется, новая создается, а `audit_logs` сохраняет состояние до и после.

Модель не получает SQL и идентификаторы базы и не может выполнить произвольный запрос. Она планирует действие, а детерминированный исполнитель применяет только разрешенную схему. Место оплаты и назначение расхода разделены: например, пополнение SIM в 7-Eleven относится к `mobile`, тогда как обычные товары без более точного назначения — к `seven_eleven`.
