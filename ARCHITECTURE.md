...
При ориентировочном курсе 14,01 KZT за THB бюджет 57 100 THB равен примерно 800 000 KZT. Дополнительные 50 000 KZT из суммы трех выплат направляются в фонд бордеррана. Получается полное распределение: `200 000 обязательства + 600 000 автомобиль + 50 000 бордерран + около 800 000 жизнь = около 1 650 000 KZT`.
...
Каждое изменение курса THB на 0,10 KZT меняет тенговую стоимость месячного лимита 57 100 THB на 5 710 KZT. Если прогноз накопления становится ниже 600 000 KZT, вечерний отчет показывает точный ожидаемый недобор.
...
Всего потрачено: 21 400 / 57 100 ฿
Осталось: 35 700 ฿ · примерно 500 157 ₸
...
4-го числа после вечернего отчета бот рассчитывает остаток категорий и присылает предложение:

```text
В категориях осталось 3 420 ฿ (47 914 ₸).
Добавить эту сумму к цели «Автомобиль»?

[Добавить к цели] [Оставить резервом] [Настроить вручную]
```

До ответа деньги не считаются накопленными. При подтверждении создается отдельная запись `goal_contribution`, а не переписываются старые расходы. Закрытый месяц неизменяем; поздняя корректировка оформляется отдельной корректирующей проводкой с аудитом.
